...
import { Description, Search, ArrowDropDown, FilterList, Edit, Save, History } from "@mui/icons-material";
...
const DoctorPatients = () => {
  ...
  const handleEditCondition = (patientId, condition) => {
    setEditableCondition({ ...editableCondition, [patientId]: condition });
    setIsEditing({ ...isEditing, [patientId]: true });
  };

  const handleSaveCondition = async (patientId) => {
    try {
      const newCondition = editableCondition[patientId];
      const timestamp = new Date().toISOString();
      const patientDoc = doc(db, "Employee", patientId);
      await updateDoc(patientDoc, {
        Condition: newCondition,
        ConditionHistory: arrayUnion({ condition: newCondition, timestamp, doctorId }),
      });
      setIsEditing({ ...isEditing, [patientId]: false });
    } catch (error) {
  ...
  const handleViewHistory = (patientId) => {
    ...
    setConditionHistory(patient.ConditionHistory || []);
                    ...
                    >
                      ...
                      <TableCell>
                        {isEditing[patient.id] ? (
                          <TextField
                            value={editableCondition[patient.id] || ''}
                            onChange={(e) => setEditableCondition({ ...editableCondition, [patient.id]: e.target.value })}
                            size="small"
                            sx={{ width: '100%' }}
                          />
                        ) : (
                          <Chip
                            label={patient.Condition || 'N/A'}
                            size="small"
                            sx={{
                              borderRadius: 1,
                              backgroundColor: alpha(theme.palette.primary.light, 0.1),
                      ...
                      <TableCell>
                        {isEditing[patient.id] ? (
                          <Tooltip title="Save" arrow>
                            <IconButton 
                              size="small"
                              onClick={() => handleSaveCondition(patient.id)}
                              sx={{
                          ...
                          <Tooltip title="Edit Condition" arrow>
                            <IconButton 
                              size="small"
                              onClick={() => handleEditCondition(patient.id, patient.Condition)}
                              sx={{
                                backgroundColor: alpha(theme.palette.primary.main, 0.1),
                                '&:hover': {
                                  backgroundColor: alpha(theme.palette.primary.main, 0.2)
                        ...
                        <Tooltip title="View Condition History" arrow>
                          <IconButton 
                            size="small"
                        ...
                        <Tooltip title="View Medical Reports" arrow>
                          <IconButton 
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              navigate(`/doctor/dashboard?employeeID=${patient.id}`);
                            }}
                            sx={{
                              backgroundColor: alpha(theme.palette.primary.main, 0.1),
                              '&:hover': {
  ...
  <DialogTitle>Condition History</DialogTitle>
        ...
        <TableBody>
          {conditionHistory.map((entry, index) => (
            <TableRow key={index}>
              ...
              <TableCell>{entry.doctorId}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
  ...
  <DialogActions>
    <Button onClick={() => setHistoryDialogOpen(false)} color="primary">Close</Button>
  </DialogActions>
</Dialog>
    </Box>
  );
